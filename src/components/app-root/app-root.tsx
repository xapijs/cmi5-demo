...
export class AppRoot {
  ...
  gameStart: Date;
  @State() sequencing: string[] = ["Four", "Two", "One", "Three"];
  numeric: number = 0;
  ...
  interactionTrueFalse(answer: boolean) {
    this.cmi5.interactionTrueFalse(
      "test1",
      "is-true-true",
      answer,
      true,
  ...
  interactionChoice(event: Event) {
    ...
    this.cmi5.interactionChoice(
      "test1",
      "first-two-choices",
      Object.keys(this.choices).filter((key) => {
  ...
  interactionLongFillIn(event: Event) {
    event.preventDefault();
    this.cmi5.interactionLongFillIn(
      "test1",
      "hello-long",
      [this.longFillIn],
      ["World"],
  ...
  @Listen("gameScore") interactionPerformance(event: CustomEvent) {
    ...
    this.cmi5.interactionPerformance(
      "test1",
      "trex-runner",
      {
        distance: score,
  ...
  interactionSequencing(event: Event) {
    event.preventDefault();
    this.cmi5.interactionSequencing(
      "test1",
      "number-order",
  ...
  reorderSequence(value: string) {
    ...
    if (index > -1) {
      let arr = [...this.sequencing];
      arr.splice(index, 1);
      this.sequencing = arr;
    } else {
      this.sequencing = [...this.sequencing, value];
    }
  }

  interactionNumeric(event: Event) {
    event.preventDefault();
    this.cmi5.interactionNumeric(
      "test1",
      "human-legs",
      this.numeric,
      { exact: 2 },
  ...
  render() {
    return [
      ...
      <main>
        <button onClick={() => this.exit()} disabled={!this.initialized}>
          Exit
        </button>
        <h2>Getters</h2>
        <button onClick={() => this.getLaunchParameters()}>
          Get Launch Parameters
        </button>
        <button
          onClick={() => this.getLaunchData()}
          disabled={!this.initialized}
        >
          Get Launch Data
        </button>
        <button
          onClick={() => this.getLearnerPreferences()}
          disabled={!this.initialized}
        >
          Get Learner Preferences
        </button>
        <h2>"cmi5 defined" Statements</h2>
        <button onClick={() => this.initialize()} disabled={this.initialized}>
          Initialize
        </button>
        <button onClick={() => this.complete()} disabled={!this.initialized}>
          Complete
        </button>
        <button onClick={() => this.pass()} disabled={!this.initialized}>
          Pass
        </button>
        <button onClick={() => this.fail()} disabled={!this.initialized}>
          Fail
        </button>
        <button onClick={() => this.terminate()} disabled={!this.initialized}>
          Terminate
        </button>
        <h2>"cmi5 allowed" Statements</h2>
        <h3>Progress</h3>
        <button onClick={() => this.progress(25)} disabled={!this.initialized}>
          Progress 25%
        </button>
        <button onClick={() => this.progress(50)} disabled={!this.initialized}>
          Progress 50%
        </button>
        <button onClick={() => this.progress(75)} disabled={!this.initialized}>
          Progress 75%
        </button>
        <h3>Interactions</h3>
        <h4>True / False</h4>
        <p>
          Q1. In a boolean context, is <code>true</code> truthy?
        </p>
        <button
          onClick={() => this.interactionTrueFalse(true)}
          disabled={!this.initialized}
        >
          True (Correct)
        </button>
        <button
          onClick={() => this.interactionTrueFalse(false)}
          disabled={!this.initialized}
        >
          False (Incorrect)
        </button>
        ...
        <form onSubmit={(event) => this.interactionFillIn(event)}>
          <label>
            <input
              type="text"
              onInput={(event) =>
                this.setFillIn((event.target as HTMLInputElement).value)
              }
            />
          </label>
          <input type="submit" value="Submit" disabled={!this.initialized} />
        ...
        <form onSubmit={(event) => this.interactionLongFillIn(event)}>
          <label>
            <textarea
              onInput={(event) =>
                this.setLongFillIn((event.target as HTMLTextAreaElement).value)
              }
            ></textarea>
          </label>
          <input type="submit" value="Submit" disabled={!this.initialized} />
        ...
        <form onSubmit={(event) => this.interactionLikert(event)}>
            ...
            <li>
              <label>
                <input
                  ...
                  value="likert0"
                  onInput={(event) =>
                    this.setLikert((event.target as HTMLInputElement).value)
                  }
                />
                Very Unsatisfied
              </label>
          ...
          <input
            type="submit"
            value="Submit"
            disabled={!this.initialized || !this.likert}
          />
        </form>
        <h4>Matching</h4>
        ...
        <form onSubmit={(event) => this.interactionSequencing(event)}>
          <button
            type="button"
            onClick={() => this.reorderSequence("One")}
            class={{ pressed: this.sequencing.indexOf("One") > -1 }}
          >
            One
          </button>
          <button
            type="button"
            onClick={() => this.reorderSequence("Two")}
            class={{ pressed: this.sequencing.indexOf("Two") > -1 }}
          >
            Two
          </button>
          <button
            type="button"
            onClick={() => this.reorderSequence("Three")}
            class={{ pressed: this.sequencing.indexOf("Three") > -1 }}
          >
            Three
          </button>
          <button
            type="button"
            onClick={() => this.reorderSequence("Four")}
            class={{ pressed: this.sequencing.indexOf("Four") > -1 }}
          >
            Four
          </button>
          <div>
          ...
          <button
            type="submit"
            value="Submit"
            disabled={!this.initialized || this.sequencing.length < 4}
          >
            Submit
          </button>
        </form>
        ...
        {this.location}
        <button
          onClick={() => this.interactionOther()}
          disabled={!this.initialized || !this.location}
        >
          Submit
        </button>
        <h3>Objectives</h3>
        ...
        <h5>Interaction</h5>
        <p>
          In a boolean context, is <code>true</code> truthy?
        </p>
        <button
          onClick={() => this.interactionTrueFalseObjective(true)}
          disabled={!this.initialized}
        >
          True (Correct)
        </button>
